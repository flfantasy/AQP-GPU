...
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__global__ void build_hashtable_p(int *dim_key, int *dim_val, int *hash_table) {
  int items[ITEMS_PER_THREAD];
...
void queryKernel(
  ...
  build_hashtable_p<128,4><<<num_blocks, 128>>>(p_partkey, p_brand1, ht_p);
...
void run(
    ...
    int* h_p_partkey, int* h_p_brand1,
