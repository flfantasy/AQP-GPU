...
__global__ void kernel(int* lo_revenue, int lo_num_entries, unsigned long long* sum,
  ...
  if (x < lo_num_entries * TIMES) {
    ...
    __shared__ unsigned int rand;
    if (laneid == 0){
      rand = curand(curand_states + (smid * 1024 + warpid * 32 + laneid));
    } else {
      curand(curand_states + (smid * 1024 + warpid * 32 + laneid));
    }
    __syncwarp();
    // __syncthreads();
    int idx = x % TIMES;
    // int loidx = rand % (lo_num_entries-31) + laneid;
    int loidx = rand % (lo_num_entries-31) + laneid;
