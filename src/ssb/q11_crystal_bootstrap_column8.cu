...
__global__ void create_BS_sample(
    int* bs_lo_orderdate, int* bs_lo_discount, int* bs_lo_quantity, int* bs_lo_extendedprice, int* bs_lo_ordtotalprice, int* bs_lo_revenue, int* bs_lo_supplycost, int* bs_lo_tax,
    int* d_lo_orderdate, int* d_lo_discount, int* d_lo_quantity, int* d_lo_extendedprice, int* d_lo_ordtotalprice, int* d_lo_revenue, int* d_lo_supplycost, int* d_lo_tax,
...
__global__ void queryKernel(int* lo_orderdate, int* lo_discount, int* lo_quantity, int* lo_extendedprice, int* lo_ordtotalprice, int* lo_revenue,  int* lo_supplycost, int* lo_tax,
...
void run(int* h_lo_orderdate, int* h_lo_discount, int* h_lo_quantity, int* h_lo_extendedprice, int* h_lo_ordtotalprice, int* h_lo_revenue, int* h_lo_supplycost, int* h_lo_tax,
  ...
  for (int i = 0; i < TIMES; i++) {
    cudaEventRecord(start, 0);
    {
      ...
      create_BS_sample<<<num_blocks, THREADS_PER_BLOCK>>>(
          bs_lo_orderdate, bs_lo_discount, bs_lo_quantity, bs_lo_extendedprice, bs_lo_ordtotalprice, bs_lo_revenue, bs_lo_supplycost, bs_lo_tax,
          d_lo_orderdate, d_lo_discount, d_lo_quantity, d_lo_extendedprice, d_lo_ordtotalprice, d_lo_revenue, d_lo_supplycost, d_lo_tax, curand_state);
    }
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&unit_time, start, stop);
    ...
    cudaEventRecord(start, 0);
    {
      ...
      queryKernel<128,4><<<num_blocks, 128>>>(
          bs_lo_orderdate, bs_lo_discount, bs_lo_quantity, bs_lo_extendedprice, bs_lo_ordtotalprice, bs_lo_revenue, bs_lo_supplycost, bs_lo_tax, d_bs_res+i);
    }
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&unit_time, start, stop);
  ...
  int num_blocks = (LO_LEN - 1) / tile_items + 1;
  queryKernel<128,4><<<num_blocks, 128>>>(
      d_lo_orderdate, d_lo_discount, d_lo_quantity, d_lo_extendedprice, d_lo_ordtotalprice, d_lo_revenue, d_lo_supplycost, d_lo_tax, d_res);
...
int main(int argc, char** argv){
  ...
  run(h_lo_orderdate, h_lo_discount, h_lo_quantity, h_lo_extendedprice, h_lo_ordtotalprice, h_lo_revenue, h_lo_supplycost, h_lo_tax, g_allocator);
