...
__global__ void createBSSampleAndProbe(
  ...
  if (x < LO_LEN) {
    ...
    if (selection_flags[0]) {
      int hash = (p_brand1[0] * 7 +  (d_year[0] - 1992)) % ((1998-1992+1) * (5*5*40));
      res[hash * 4] = d_year[0];
      res[hash * 4 + 1] = p_brand1[0];
      atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 4 + 2]), (long long)(revenue[0]));
    }
  }
}
...
void createBSSampleAndRunQuery(
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cout << "=" << flush;
    {
      ...
      build_hashtable_p<128,4><<<num_blocks, 128>>>(p_category, p_partkey, p_brand1, d_ht_p);
