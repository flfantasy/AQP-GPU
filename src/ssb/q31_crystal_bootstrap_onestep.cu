...
__global__ void createBSSampleAndProbe(
  ...
  if (x < LO_LEN) {
    ...
    if (selection_flags[0]) {
      int hash = (s_nation[0] * 25 * 7  + c_nation[0] * 7 +  (d_year[0] - 1992)) % ((1998-1992+1) * 25 * 25);
      res[hash * 6] = c_nation[0];
      res[hash * 6 + 1] = s_nation[0];
      res[hash * 6 + 2] = d_year[0];
      atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 6 + 4]), (long long)(revenue[0]));
    }
  }
}
...
void createBSSampleAndRunQuery(
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cout << "=" << flush;
    {
      ...
      build_hashtable_s<128,4><<<num_blocks, 128>>>(s_region, s_suppkey, s_nation, d_ht_s);
