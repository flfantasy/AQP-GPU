  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if (threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items) {
      if (selection_flags[ITEM]) {
        ...
        atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 4 + 2]), (long long)(revenue[ITEM] - items[ITEM]) * linecount[ITEM]);
      }
    }
  }
}
...
void queryKernel(
  ...
  build_hashtable_p<128,4><<<num_blocks, 128>>>(p_mfgr, p_partkey, ht_p);
