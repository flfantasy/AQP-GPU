...
static void resample(string pathToDatabase){
  ...
  srand((unsigned)time(NULL));
  {
    hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    {
      ...
      {
        bootstrapTable.setTableName("q33_bs_" + to_string(i));
...
static void run(string pathToDatabase){
  ...
  hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    ...
    for (size_t i = 0; i < TIMES; i++){
      hyperapi::Result rowsInTable = connection.executeQuery(
        "select c_city, s_city, d_year, sum(lo_revenue) as revenue "
        "from q33_bs_" + to_string(i) + ", customer, supplier, ddate "
        "where lo_custkey = c_custkey "
        "and lo_suppkey = s_suppkey "
        "and lo_orderdate = d_datekey "
        "and (c_city = 231 or c_city = 235) "
...
int main(int argc, char** argv) {
  const string pathToDatabase = "test/ssb/data/s1.hyper.q33";
  // 拷贝原始hyper文件
  system("cp /home/zhaoh/crystal/test/ssb/data/s1.hyper /home/zhaoh/crystal/test/ssb/data/s1.hyper.q33");
  resample(pathToDatabase);
  run(pathToDatabase);
  // 删除残留hyper文件
  system("rm /home/zhaoh/crystal/test/ssb/data/s1.hyper.q33");
  return 0;
}