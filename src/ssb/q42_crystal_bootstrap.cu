...
__global__ void create_BS_sample(
    int* bs_lo_orderdate, int* bs_lo_custkey, int* bs_lo_suppkey, int* bs_lo_partkey, int* bs_lo_revenue, int* bs_lo_supplycost,
    int* d_lo_orderdate, int* d_lo_custkey, int* d_lo_suppkey, int* d_lo_partkey, int* d_lo_revenue, int* d_lo_supplycost,
  ...
  for (int i = 0; i < TIMES; i++) {
    cudaEventRecord(start, 0);
    {
      ...
      create_BS_sample<<<num_blocks, THREADS_PER_BLOCK>>>(
          bs_lo_orderdate, bs_lo_custkey, bs_lo_suppkey, bs_lo_partkey, bs_lo_revenue, bs_lo_supplycost,
