  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM)) < num_tile_items) {
      if (selection_flags[ITEM]) {
        ...
        atomicAdd(&res[hash * 4 + 3], revenue[ITEM] * linecount[ITEM]);
      }
    }
  }
}
...
void queryKernel(
    ...
    int* c_custkey, int* c_nation, int* c_city,
  ...
  build_hashtable_c<128,4><<<num_blocks, 128>>>(c_nation, c_custkey, c_city, ht_c);
...
void run(
    ...
    int* h_c_custkey, int* h_c_nation, int* h_c_city,
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cudaEventRecord(start, 0);
    {
      ...
      queryKernel(
          ...
          d_c_custkey, d_c_nation, d_c_city,
...
int main(int argc, char** argv){
  ...
  run(
      ...
      h_c_custkey, h_c_nation, h_c_city,
