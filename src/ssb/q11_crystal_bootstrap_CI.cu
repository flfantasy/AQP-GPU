...
void run(int* h_lo_orderdate, int* h_lo_discount, int* h_lo_quantity, int* h_lo_extendedprice, 
  ...
  int idx1 = TIMES * 0.01;
  int idx2 = TIMES * 0.99;
  *low = (double)((long long)h_bs_res[idx1]-(long long)h_res)/h_res;
  *high = (double)((long long)h_bs_res[idx2]-h_res)/h_res;
  cout << h_res << " (" << *low << ", " << *high << ")" << endl; 
...
int main(int argc, char** argv){
  ...
  for (int i = 0; i < 100; i++)
  {
    run(h_lo_orderdate, h_lo_discount, h_lo_quantity, h_lo_extendedprice, low+i, high+i, g_allocator);
    avgl += low[i];
    avgh += high[i];
  }
  avgl /= 100;
  ...
  {
    varl += (low[i]-avgl) * (low[i]-avgl);
    varh += (high[i]-avgh) * (high[i]-avgh);
  }
  varl /= 100;
  varh /= 100;
  cout << "varl: " << varl << endl;
