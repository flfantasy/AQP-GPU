...
__global__ void build_hashtable_c(int* filter_col, int *dim_key, int *hash_table) {
...
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__global__ void build_hashtable_s(int *filter_col, int *dim_key, int* dim_val, int *hash_table) {
  int items[ITEMS_PER_THREAD];
...
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__global__ void probe(int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_partkey, int* lo_revenue, int* lo_supplycost, int* lo_linecount,
  ...
  BlockProbeAndPHT_1<int, BLOCK_THREADS, ITEMS_PER_THREAD>(items, selection_flags, ht_c, C_SLOT_LEN, num_tile_items);

  BlockLoad<int, BLOCK_THREADS, ITEMS_PER_THREAD>(lo_partkey + tile_offset, items, num_tile_items);
  BlockProbeAndPHT_2<int, int, BLOCK_THREADS, ITEMS_PER_THREAD>(items, p_category, selection_flags,
      ht_p, P_SLOT_LEN, num_tile_items);
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if (threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items) {
      if (selection_flags[ITEM]) {
        ...
        int hash = ((d_year[ITEM] - 1992) * 25 * 25 + s_nation[ITEM] * 25 + p_category[ITEM]) % ((1998-1992+1) * 25 * 25);
        res[hash * 6] = d_year[ITEM];
        res[hash * 6 + 1] = s_nation[ITEM];
        res[hash * 6 + 2] = p_category[ITEM];
        atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 6 + 4]), (long long)((revenue[ITEM] - items[ITEM]) * linecount[ITEM]));
      }
    }
  }
}
...
void queryKernel(
    int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_partkey, int* lo_revenue, int* lo_supplycost, int* lo_linecount,
    int* c_custkey, int* c_region,
    int* s_suppkey, int* s_nation, int* s_region,
    int* p_partkey, int* p_mfgr, int* p_category,
    int* d_datekey, int* d_year,
    int* ht_c, int* ht_s, int* ht_p, int* ht_d, int* res) {

  int items_per_tile = 128*4;
  int num_blocks = (C_LEN - 1) / items_per_tile + 1;
  build_hashtable_c<128,4><<<num_blocks, 128>>>(c_region, c_custkey, ht_c);
  num_blocks = (S_LEN - 1) / items_per_tile + 1;
  build_hashtable_s<128,4><<<num_blocks, 128>>>(s_region, s_suppkey, s_nation, ht_s);
  num_blocks = (P_LEN - 1) / items_per_tile + 1;
  build_hashtable_p<128,4><<<num_blocks, 128>>>(p_mfgr, p_partkey, p_category, ht_p);
  ...
  probe<128,4><<<num_blocks, 128>>>(
      lo_orderdate, lo_custkey, lo_suppkey, lo_partkey, lo_revenue, lo_supplycost, lo_linecount,
      ht_c, ht_s, ht_p, ht_d,
      res);
}
...
void run(
    ...
    int* h_c_custkey, int* h_c_region,
    int* h_s_suppkey, int* h_s_nation, int* h_s_region,
    int* h_p_partkey, int* h_p_mfgr, int* h_p_category,
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cudaEventRecord(start, 0);
    {
      CubDebugExit(cudaMemset(d_res, 0, res_array_size * sizeof(int)));
      queryKernel(
          d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_partkey, d_lo_revenue, d_lo_supplycost, lo_linecount,
          d_c_custkey, d_c_region,
          d_s_suppkey, d_s_nation, d_s_region,
          d_p_partkey, d_p_mfgr, d_p_category,
          d_d_datekey, d_d_year,
          d_ht_c, d_ht_s, d_ht_p, d_ht_d, d_res);
      CubDebugExit(cudaMemcpy(h_res, d_res, res_array_size * sizeof(int), cudaMemcpyDeviceToHost));
      for (int i = 0; i < res_size; i++) {
        if (h_res[6*i] != 0) {
          int d_year = h_res[6*i];
          int s_nation = h_res[6*i + 1];
          int p_category = h_res[6*i + 2];
          long long sum = reinterpret_cast<unsigned long long*>(&h_res[6*i + 4])[0];
          tuple<int, int, int> t(d_year, s_nation, p_category);
  ...
  CubDebugExit(cudaMemset(d_res, 0, res_array_size * sizeof(int)));
  queryKernel(
      d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_partkey, d_lo_revenue, d_lo_supplycost, lo_linecount_origin,
      d_c_custkey, d_c_region,
      d_s_suppkey, d_s_nation, d_s_region,
      d_p_partkey, d_p_mfgr, d_p_category,
      d_d_datekey, d_d_year,
...
int main(int argc, char** argv){
  ...
  run(
      ...
      h_c_custkey, h_c_region,
      h_s_suppkey, h_s_nation, h_s_region,
      h_p_partkey, h_p_mfgr, h_p_category,
