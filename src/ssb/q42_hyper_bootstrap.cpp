...
static void resample(string pathToDatabase){
  ...
  int *bs_lo_supplycost = new int[LO_LEN];
  ...
  memset(bs_lo_supplycost, 0, sizeof(bs_lo_supplycost));
  ...
  srand((unsigned)time(NULL));
  {
    hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    {
      ...
      {
        bootstrapTable.setTableName("q42_bs_" + to_string(i));
        ...
        catalog.createTableIfNotExists(bootstrapTable);
        {
          ...
          for (size_t j = 0; j < LO_LEN; j++){
            ...
            bs_lo_supplycost[j] = h_lo_supplycost[loidx];
          ...
          for (size_t j = 0; j < LO_LEN; j++)
          {
            inserter.addRow(bs_lo_custkey[j], bs_lo_suppkey[j], bs_lo_partkey[j], bs_lo_orderdate[j], bs_lo_supplycost[j], bs_lo_revenue[j]);
          }
          inserter.execute();
          inserter.close();
        }
...
static void run(string pathToDatabase){
  ...
  hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    ...
    for (const hyperapi::Row& row : rowsInTable) {
      ...
      int s_nation = (*(++iterator)).get<int>();
      int p_category = (*(++iterator)).get<int>();
    ...
    for (size_t i = 0; i < TIMES; i++){
      hyperapi::Result rowsInTable = connection.executeQuery(
        "select d_year, s_nation, p_category, sum(lo_revenue - lo_supplycost) as profit "
        "from q42_bs_" + to_string(i) + ", supplier, customer, part, ddate "
        "where lo_custkey = c_custkey "
        "and lo_suppkey = s_suppkey "
        "and lo_partkey = p_partkey "
        "and lo_orderdate = d_datekey "
...
int main(int argc, char** argv) {
  const string pathToDatabase = "test/ssb/data/s1.hyper.q42";
  // 拷贝原始hyper文件
  system("cp /home/zhaoh/crystal/test/ssb/data/s1.hyper /home/zhaoh/crystal/test/ssb/data/s1.hyper.q42");
  resample(pathToDatabase);
  run(pathToDatabase);
  // 删除残留hyper文件
  system("rm /home/zhaoh/crystal/test/ssb/data/s1.hyper.q42");
  return 0;
}