  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if (threadIdx.x + (BLOCK_THREADS * ITEM) < num_tile_items) {
      if (selection_flags[ITEM]) {
        ...
        int hash = ((d_year[ITEM] - 1992) * 250 * 1000 + s_city[ITEM] * 1000 + p_brand1[ITEM]) % ((1998-1992+1) * 250 * 1000);
        res[hash * 4] = d_year[ITEM];
        res[hash * 4 + 1] = s_city[ITEM];
        res[hash * 4 + 2] = p_brand1[ITEM];
...
void queryKernel(
    ...
    int* s_suppkey, int* s_nation, int* s_city,
  ...
  build_hashtable_s<128,4><<<num_blocks, 128>>>(s_nation, s_suppkey, s_city, ht_s);
...
void run(
    ...
    int* h_s_suppkey, int* h_s_nation, int* h_s_city,
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cudaEventRecord(start, 0);
    {
      ...
      queryKernel(
          ...
          d_s_suppkey, d_s_nation, d_s_city,
      ...
      for (int i = 0; i < res_size; i++) {
        if (h_res[4*i] != 0) {
          ...
          int s_city = h_res[4*i + 1];
          int p_brand1 = h_res[4*i + 2];
          int sum = h_res[4*i + 3];
          tuple<int, int, int> t(d_year, s_city, p_brand1);
...
int main(int argc, char** argv){
  ...
  run(
      ...
      h_s_suppkey, h_s_nation, h_s_city,
