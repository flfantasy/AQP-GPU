...
__global__ void probe(
    int* lo_orderdate, int* lo_suppkey, int* lo_partkey, int* lo_revenue, int* lo_linecount,
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM)) < num_tile_items) {
      if (selection_flags[ITEM]) {
        ...
        atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 4 + 2]), (long long)(revenue[ITEM] * linecount[ITEM]));
      }
    }
  }
}
...
void queryKernel(
  ...
  probe<128,4><<<num_blocks, 128>>>(
      lo_orderdate, lo_suppkey, lo_partkey, lo_revenue, lo_linecount,
      ht_s, ht_p, ht_d,
      res);
...
void run(
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cudaEventRecord(start, 0);
    {
      CubDebugExit(cudaMemset(d_res, 0, res_array_size * sizeof(int)));
      queryKernel(
          d_lo_orderdate, d_lo_suppkey, d_lo_partkey, d_lo_revenue, lo_linecount,
  ...
  CubDebugExit(cudaMemset(d_res, 0, res_array_size * sizeof(int)));
  queryKernel(
      d_lo_orderdate, d_lo_suppkey, d_lo_partkey, d_lo_revenue, lo_linecount_origin,
      d_s_suppkey, d_s_region,
      d_p_partkey, d_p_brand1, d_p_category,
      d_d_datekey, d_d_year,
      d_ht_s, d_ht_p, d_ht_d, d_res);
