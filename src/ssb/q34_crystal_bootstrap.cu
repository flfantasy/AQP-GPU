...
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__global__ void build_hashtable_d(int* filter_col, int *dim_key, int *dim_val, int *hash_table) {
  int items[ITEMS_PER_THREAD];
  int items2[ITEMS_PER_THREAD];
  int selection_flags[ITEMS_PER_THREAD];
...
void queryKernel(
    ...
    int* d_datekey, int* d_year, int* d_d_yearmonthnum,
  ...
  build_hashtable_d<128,4><<<num_blocks, 128>>>(d_d_yearmonthnum, d_datekey, d_year, ht_d);
...
void run(
    ...
    int* h_d_datekey, int* h_d_year, int* h_d_yearmonthnum,
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cudaEventRecord(start, 0);
    {
      ...
      queryKernel(
          ...
          d_d_datekey, d_d_year, d_d_yearmonthnum,
...
int main(int argc, char** argv){
  ...
  run(
      ...
      h_d_datekey, h_d_year, h_d_yearmonthnum,
      g_allocator);
