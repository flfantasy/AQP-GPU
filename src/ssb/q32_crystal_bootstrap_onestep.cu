...
__global__ void createBSSampleAndProbe(
  ...
  if (x < LO_LEN) {
    ...
    if (selection_flags[0]) {
      int hash = (s_city[0] * 250 * 7  + c_city[0] * 7 +  (d_year[0] - 1992)) % ((1998-1992+1) * 250 * 250);
      res[hash * 4] = c_city[0];
      res[hash * 4 + 1] = s_city[0];
      res[hash * 4 + 2] = d_year[0];
      atomicAdd(&res[hash * 4 + 3], revenue[0]);
    }
  }
}
...
void createBSSampleAndRunQuery(
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cout << "=" << flush;
    {
      int items_per_tile = 128*4;
      int num_blocks = (C_LEN - 1) / items_per_tile + 1;
      build_hashtable_c<128,4><<<num_blocks, 128>>>(c_nation, c_custkey, c_city, d_ht_c);
      num_blocks = (S_LEN - 1) / items_per_tile + 1;
      build_hashtable_s<128,4><<<num_blocks, 128>>>(s_nation, s_suppkey, s_city, d_ht_s);
      num_blocks = (D_LEN - 1) / items_per_tile + 1;
