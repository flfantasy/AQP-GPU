...
__global__ void create_BS_sample(
    ...
    for (size_t i = 0; i < LO_LEN; i++) {
      unsigned int rand = curand(curand_states);
      int loidx = rand % LO_LEN;
      bs_lo_orderdate[i] = d_lo_orderdate[loidx];
      bs_lo_discount[i] = d_lo_discount[loidx];
      bs_lo_quantity[i] = d_lo_quantity[loidx];
      bs_lo_extendedprice[i] = d_lo_extendedprice[loidx];
    }
}
...
void run(int* h_lo_orderdate, int* h_lo_discount, int* h_lo_quantity, int* h_lo_extendedprice, 
  ...
  for (int i = 0; i < TIMES; i++) {
    cudaEventRecord(start, 0);
    {
      create_BS_sample<<<1, 1>>>(bs_lo_orderdate, bs_lo_discount, bs_lo_quantity, bs_lo_extendedprice, d_lo_orderdate, d_lo_discount, d_lo_quantity, d_lo_extendedprice, curand_state);
    }
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&unit_time, start, stop);
