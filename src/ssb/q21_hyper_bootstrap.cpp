...
static void resample(string pathToDatabase){
  ...
  srand((unsigned)time(NULL));
  {
    hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    {
      ...
      {
        bootstrapTable.setTableName("q21_bs_" + to_string(i));
...
static void run(string pathToDatabase){
  ...
  hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    ...
    for (size_t i = 0; i < TIMES; i++){
      hyperapi::Result rowsInTable = connection.executeQuery(
        "select sum(lo_revenue), d_year, p_brand1 "
        "from q21_bs_" + to_string(i) + ", ddate, part, supplier "
        "where lo_orderdate = d_datekey "
        "and lo_partkey = p_partkey "
        "and lo_suppkey = s_suppkey "
        "and p_category = 1 "
...
int main(int argc, char** argv) {
  const string pathToDatabase = "test/ssb/data/s1.hyper.q21";
  // 拷贝原始hyper文件
  system("cp /home/zhaoh/crystal/test/ssb/data/s1.hyper /home/zhaoh/crystal/test/ssb/data/s1.hyper.q21");
  resample(pathToDatabase);
  run(pathToDatabase);
  // 删除残留hyper文件
  system("rm /home/zhaoh/crystal/test/ssb/data/s1.hyper.q21");
  return 0;
}