...
__global__ void create_BS_sample(
    int* bs_lo_discount, int* bs_lo_extendedprice,
    int* d_lo_discount, int* d_lo_extendedprice,
...
__global__ void queryKernel(int* lo_discount, int* lo_extendedprice,
...
void run(int* h_lo_discount, int* h_lo_extendedprice, 
  ...
  for (int i = 0; i < TIMES; i++) {
    cudaEventRecord(start, 0);
    {
      ...
      create_BS_sample<<<num_blocks, THREADS_PER_BLOCK>>>(bs_lo_discount, bs_lo_extendedprice, d_lo_discount, d_lo_extendedprice, curand_state);
    }
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&unit_time, start, stop);
    ...
    cudaEventRecord(start, 0);
    {
      ...
      queryKernel<128,4><<<num_blocks, 128>>>(bs_lo_discount, bs_lo_extendedprice, d_bs_res+i);
    }
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&unit_time, start, stop);
  ...
  int tile_items = 128*4;
  int num_blocks = (LO_LEN - 1) / tile_items + 1;
  queryKernel<128,4><<<num_blocks, 128>>>(d_lo_discount, d_lo_extendedprice, d_res);
...
int main(int argc, char** argv){
  ...
  run(h_lo_discount, h_lo_extendedprice, g_allocator);
