...
template<int BLOCK_THREADS, int ITEMS_PER_THREAD>
__global__ void probe(int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_revenue, int* lo_linecount,
  ...
  for (int ITEM = 0; ITEM < ITEMS_PER_THREAD; ++ITEM) {
    if ((threadIdx.x + (BLOCK_THREADS * ITEM)) < num_tile_items) {
      if (selection_flags[ITEM]) {
        int hash = (s_nation[ITEM] * 25 * 7  + c_nation[ITEM] * 7 +  (d_year[ITEM] - 1992)) % ((1998-1992+1) * 25 * 25);
        res[hash * 6] = c_nation[ITEM];
        res[hash * 6 + 1] = s_nation[ITEM];
        res[hash * 6 + 2] = d_year[ITEM];
        atomicAdd(reinterpret_cast<unsigned long long*>(&res[hash * 6 + 4]), (long long)(revenue[ITEM] * linecount[ITEM]));
      }
    }
  }
}
...
void queryKernel(
    int* lo_orderdate, int* lo_custkey, int* lo_suppkey, int* lo_revenue, int* lo_linecount,
    int* c_custkey, int* c_region, int* c_nation,
  ...
  build_hashtable_c<128,4><<<num_blocks, 128>>>(c_region, c_custkey, c_nation, ht_c);
  ...
  probe<128,4><<<num_blocks, 128>>>(
      lo_orderdate, lo_custkey, lo_suppkey, lo_revenue, lo_linecount,
      ht_c, ht_s, ht_d,
      res);
}
...
void run(
    ...
    int* h_c_custkey, int* h_c_region, int* h_c_nation,
  ...
  for (int i = 0; i < TIMES; i++) {
    ...
    cudaEventRecord(start, 0);
    {
      ...
      CubDebugExit(cudaMemset(d_res, 0, res_array_size * sizeof(int)));
      queryKernel(
          d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_revenue, lo_linecount,
          d_c_custkey, d_c_region, d_c_nation,
      ...
      for (int i = 0; i < res_size; i++) {
        if (h_res[6*i] != 0) {
          ...
          int c_nation = h_res[6*i];
          int s_nation = h_res[6*i + 1];
          int d_year = h_res[6*i + 2];
          long long sum = reinterpret_cast<long long*>(&h_res[6*i + 4])[0];
  ...
  CubDebugExit(cudaMemset(d_res, 0, res_array_size * sizeof(int)));
  queryKernel(
      d_lo_orderdate, d_lo_custkey, d_lo_suppkey, d_lo_revenue, lo_linecount_origin,
      d_c_custkey, d_c_region, d_c_nation,
      d_s_suppkey, d_s_nation, d_s_region,
      d_d_datekey, d_d_year,
      d_ht_c, d_ht_s, d_ht_d, d_res);
...
int main(int argc, char** argv){
  ...
  run(
      ...
      h_c_custkey, h_c_region, h_c_nation,
