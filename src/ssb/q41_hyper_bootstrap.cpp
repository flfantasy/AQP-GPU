...
static void resample(string pathToDatabase){
  ...
  srand((unsigned)time(NULL));
  {
    hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    {
      ...
      {
        bootstrapTable.setTableName("q41_bs_" + to_string(i));
...
static void run(string pathToDatabase){
  ...
  hyperapi::HyperProcess hyper(hyperapi::Telemetry::SendUsageDataToTableau);
    ...
    for (size_t i = 0; i < TIMES; i++){
      hyperapi::Result rowsInTable = connection.executeQuery(
        "select d_year, c_nation, sum(lo_revenue - lo_supplycost) as profit "
        "from q41_bs_" + to_string(i) + ", supplier, customer, part, ddate "
        "where lo_custkey = c_custkey "
        "and lo_suppkey = s_suppkey "
        "and lo_partkey = p_partkey "
        "and lo_orderdate = d_datekey "
...
int main(int argc, char** argv) {
  const string pathToDatabase = "test/ssb/data/s1.hyper.q41";
  // 拷贝原始hyper文件
  system("cp /home/zhaoh/crystal/test/ssb/data/s1.hyper /home/zhaoh/crystal/test/ssb/data/s1.hyper.q41");
  resample(pathToDatabase);
  run(pathToDatabase);
  // 删除残留hyper文件
  system("rm /home/zhaoh/crystal/test/ssb/data/s1.hyper.q41");
  return 0;
}