...
void run(
  ...
  int* h_bs_lo_partkey = new int[LO_LEN];
  int* h_bs_lo_revenue = new int[LO_LEN];
  int* h_bs_lo_supplycost = new int[LO_LEN];
  ...
  for (int i = 0; i < TIMES; i++) {
    cudaEventRecord(start, 0);
    {
      ...
      CubDebugExit(cudaMemcpy(h_bs_lo_partkey, bs_lo_partkey, LO_LEN * sizeof(int), cudaMemcpyDeviceToHost));
      CubDebugExit(cudaMemcpy(h_bs_lo_revenue, bs_lo_revenue, LO_LEN * sizeof(int), cudaMemcpyDeviceToHost));
      CubDebugExit(cudaMemcpy(h_bs_lo_supplycost, bs_lo_supplycost, LO_LEN * sizeof(int), cudaMemcpyDeviceToHost));
    }
    cudaEventRecord(stop, 0);
    cudaEventSynchronize(stop);
    cudaEventElapsedTime(&unit_time, start, stop);
