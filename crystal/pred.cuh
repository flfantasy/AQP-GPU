...
// 判断items是否符合谓词要求，存入和items对应的flags数组里
...
// 如果num_items不足一个tile，调用重载的BlockPredDirect
template<typename T, typename SelectOp, int BLOCK_THREADS, int ITEMS_PER_THREAD>
...
// 判断items是否符合谓词要求，并且原本flag为1，存入和items对应的flags数组里
template<typename T, typename SelectOp, int BLOCK_THREADS, int ITEMS_PER_THREAD>
...
// 如果num_items不足一个tile，调用重载的BlockPredAndDirect
template<typename T, typename SelectOp, int BLOCK_THREADS, int ITEMS_PER_THREAD>
...
// 判断items是否符合谓词要求，或者原本flag为1，存入和items对应的flags数组里
template<typename T, typename SelectOp, int BLOCK_THREADS, int ITEMS_PER_THREAD>
...
// 如果num_items不足一个tile，调用重载的BlockPredOrDirect
template<typename T, typename SelectOp, int BLOCK_THREADS, int ITEMS_PER_THREAD>
...
struct LessThan
{
  ...
  LessThan(T _compare) : compare(_compare) {}
...
__device__ __forceinline__ void BlockPredGT(
    T  (&items)[ITEMS_PER_THREAD],      // 谓词对象1的数组
    T compare,          // 谓词对象2
    int  (&selection_flags)[ITEMS_PER_THREAD],    // 标记数组中该值是否被选中
    int num_items     // 该tile的item数量
    ) {
